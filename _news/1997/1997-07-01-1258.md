---
layout: post
title: 'Seeing is Believing&#58; Visual Proofs of the Pythagorean Theorem'
date: 1997-07-01
tags: paper
categories:
  - papers
  - Visualization
tabs: true
image: seeing.jpg-srcw.jpg
authors:
  - Davidson
  - Kauffman
---

## Seeing is Believing: Visual Proofs of the Pythagorean Theorem
**Davidson, D.,Kauffman, L.**

[![image](/images/seeing.jpg-srcw.jpg){:style="max-width: 100%"}](/images/seeing.jpg-srcw.jpg)
- Caption: The Pythagorean Proposition
- Credit: D. Davidson, EVL

The Pythagorean theorem states that for any right triangle the area of the smallest square (AxA) plus (+) the area of the middle sized square (BxB) will always equal (=) the area of the largest square (CxC). Thus (A<sup>2</sup> + B<sup>2</sup> = C<sup>2</sup>).<br><br>But this simple algebraic formula, while truthful, reveals nothing of the elegant visual constructions devised for its demonstration.

**Citation**: Davidson, D., Kauffman, L., Seeing is Believing: Visual Proofs of the Pythagorean Theorem, *Quantum*, vol 7, no 6, pp. 24-27, 1997-07-01.